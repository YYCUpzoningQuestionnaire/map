/* voter_guide.js — Striped ward fills + mayor at City Hall + Ward Drawer ("See all") */
...
window.addEventListener('DOMContentLoaded', async () => {
  ...
  // --- FIX: hoist these so they exist before any function body uses them ---
  let currentFilteredByWard = new Map();
  let drawer, drawerOverlay, drawerTitle, drawerIssueSel, drawerAnsSel, drawerSearch, drawerTableBody, currentWardKey = null;
  ...
  function cleanSurveyFromPapa(papa){
    ...
    const header0=(data[0]||[]).map(h=>String(h??'')); const header1=(data[1]||[]).map(h=>String(h??''));
    ...
    const ffillHeaders=[]; for(let i=0;i<header0.length;i++){ const h=header0[i]; ffillHeaders[i]=(h && !/^Unnamed/i.test(h)) ? h : (i>0 ? ffillHeaders[i-1] : ''); }
    const skipIdx=new Set(); if(wardSlice) for(let i=wardSlice.start;i<wardSlice.end;i++) skipIdx.add(i);

    const groups={}; ffillHeaders.forEach((h,i)=>{ if(!h||skipIdx.has(i)) return; const k=h.trim(); (groups[k]??=[]).push(i); });
    ...
    if(header1IsChoices){
      for(const [g,idxs] of Object.entries(groups)){
        ...
        choiceIndexByGroup[g]=bucket; commentForIssue[g]=`__COMMENT::${g}`;
      }
    }

    const dataStart=header1IsChoices?2:1; const rows=[];
    for(let r=dataStart;r<data.length;r++){
      const arr=data[r]; if(!Array.isArray(arr)) continue; const obj={};
      for(const [g,idxs] of Object.entries(groups)){
        if(header1IsChoices){
          const bucket=choiceIndexByGroup[g]||{yes:[],no:[],undecided:[],comment:[]};
          let ans=''; for(const label of ['yes','no','undecided']){ for(const i of (bucket[label]||[])){ const v=arr[i]; if(v!=null && String(v).trim()!==''){ ans=label[0].toUpperCase()+label.slice(1); break; } } if(ans) break; }
          obj[g]=ans;
          const cField=commentForIssue[g]; let comm=''; for(const i of (bucket.comment||[])){ const v=arr[i]; if(v!=null && String(v).trim()!==''){ comm=String(v).trim(); break; } } obj[cField]=comm;
        }else{
          let chosen=''; for(const idx of idxs){ const v=arr[idx]; if(v!=null && String(v).trim()!==''){ chosen=String(v).trim(); break; } } obj[g]=chosen;
        }
      }
      if(wardSlice){
        let wardVal=''; for(let c=wardSlice.start;c<wardSlice.end;c++){ const v=normalize(arr[c]); if(!v) continue; wardVal = (header1IsChoices && header1[c]) ? normalize(header1[c]) : (normalize(header0[c])||v); break; }
        obj['__Ward']=wardVal;
      }
    ...
    const ynu=new Set(['yes','no','undecided']); const issueColumns=[];
    for(const c of cols){ let nonEmpty=0, ynuCount=0; for(const row of rows){ const v=normalize(row[c]); if(!v) continue; nonEmpty++; if(ynu.has(lc(v))) ynuCount++; } if(nonEmpty>0 && ynuCount/nonEmpty>=0.7) issueColumns.push(c); }
  ...
  // --- Map init ---
  const mapEl=document.getElementById('map'); if(!mapEl){ console.error('Missing <div id="map">'); return; }
  const map=L.map('map');
  L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png',{attribution:'© OpenStreetMap contributors'}).addTo(map);
  const polygonLayer=L.geoJSON(null,{style:()=>({color:'#555',weight:1,fillOpacity:0.35})}).addTo(map);
  const markerGroup=L.layerGroup().addTo(map);

  const wardsGeo=await loadJSON(WARDS_PATH); polygonLayer.addData(wardsGeo);
  ...
  function ensureDefs(){ const svg = getSVGRoot(); if(!svg) return null; let defs = svg.querySelector('defs'); if(!defs){ defs = document.createElementNS(SVGNS,'defs'); svg.insertBefore(defs, svg.firstChild); } return defs; }

  // Ward centers
  const wardCenters=[]; for(const f of wardsGeo.features){
    const p=f.properties||{};
    const key = p.ward_num ? String(parseInt(String(p.ward_num),10)) : (String((p.label||p.WARD||p.Ward||p.name||p.id||'')).match(/\d+/)||[''])[0];
    const disp=normalize(p.label)||(key?`Ward ${key}`:'(unknown ward)'); let lat=0,lng=0;
  ...
  for(const row of survey.rows){
    ...
    if(!k) continue; if(!byWardAll.has(k)) byWardAll.set(k,[]); byWardAll.get(k).push(row);
  }

  // --- Drawer UI FIRST (so handlers & overlay exist before first apply) ---
  buildDrawerUI();

  // --- Top-left controls (these call applyFilters immediately) ---
  injectControlUI();
  ...
  function injectControlUI(){
    ...
    const title=document.createElement('div'); Object.assign(title.style,{fontWeight:'600',marginBottom:'6px'}); title.textContent='Issue Filter';
    const issueSel=document.createElement('select'); issueSel.style.marginRight='6px'; issueSel.style.maxWidth='360px'; issueSel.title='Issue';
    ...
    const ansSel=document.createElement('select'); ansSel.style.marginLeft='6px';
    ...
    const legend=document.createElement('div'); legend.style.marginTop='8px'; legend.style.color='#444';
    ...
    wrap.appendChild(title); wrap.appendChild(issueSel); wrap.appendChild(ansSel); wrap.appendChild(legend);
  ...
  function ensurePattern(id, segments){
    ...
    let pat = defs.querySelector(`#${CSS.escape(id)}`); if (pat) { while(pat.firstChild) pat.removeChild(pat.firstChild); }
  ...
  function setLayerPatternFill(layer, patternId){
    ...
    path.setAttribute('fill', `url(#${patternId})`); path.setAttribute('fill-opacity', '0.45'); path.setAttribute('stroke', '#555'); path.setAttribute('stroke-width', '1');
  }
  ...
  function applyFilters(issue, wantValue){
    ...
    for(const [k,rows] of byWardAll.entries()){
      let list=rows;
      if(issue){ list=rows.filter(r=>{ const ans=ynuNormalize(r[issue]); if(!wantValue) return ans!==''; return ans===lc(wantValue); }); }
      byWard.set(k, list);
    }
    let mayorFiltered = mayorAll; if(issue){ mayorFiltered = mayorAll.filter(r=>{ const ans=ynuNormalize(r[issue]); if(!wantValue) return ans!==''; return ans===lc(wantValue); }); }
    ...
    requestAnimationFrame(() => {
      polygonLayer.eachLayer(layer=>{
        ...
        if(issue){ for(const r of rows){ const a=ynuNormalize(r[issue]); if(a==='yes') yes++; else if(a==='no') no++; else if(a==='undecided') und++; } }
        ...
        const patId = `wardStripe-${key}-${hashSegments(segments)}`; ensurePattern(patId, segments); setLayerPatternFill(layer, patId);
      });
    });
    ...
    for(const wc of wardCenters){
      ...
      const items=list.map(r=>{
        const name=buildName(r); const ans = issue ? (r[issue]||'') : '';
      ...
      const seeAllBtn = `<div style="margin-top:8px"><button
         ...
         style="border:1px solid #bbb;background:#fff;border-radius:8px;padding:4px 8px;cursor:pointer;font-size:12px">
    ...
    L.marker([CITY_HALL.lat, CITY_HALL.lng], { icon: mayorIcon, zIndexOffset: 1500 }).addTo(markerGroup).bindPopup(htmlMayor);

    updateInfo(byWard, issue, wantValue, mayorFiltered.length);
    currentFilteredByWard = byWard; // expose for drawer
  }
  ...
  function buildDrawerUI(){
    ...
    Object.assign(drawerOverlay.style,{ position:'absolute', inset:'0', background:'rgba(0,0,0,0.25)', display:'none', zIndex:1500 });
    map.getContainer().appendChild(drawerOverlay);
    drawerOverlay.addEventListener('click', closeWardDrawer);

    // drawer
    drawer = document.createElement('div');
    Object.assign(drawer.style,{ position:'absolute', top:'0', right:'0', height:'100%', width:'420px', background:'white', borderLeft:'1px solid #ccc', boxShadow:'-2px 0 10px rgba(0,0,0,0.12)', transform:'translateX(100%)', transition:'transform 150ms ease-out', zIndex:1600, display:'flex', flexDirection:'column' });
    map.getContainer().appendChild(drawer);

    // header
    const header = document.createElement('div'); Object.assign(header.style,{padding:'10px 12px', borderBottom:'1px solid #eee', display:'flex', alignItems:'center', gap:'8px'});
    const closeBtn = document.createElement('button'); closeBtn.textContent = '✕'; Object.assign(closeBtn.style,{border:'1px solid #bbb', background:'#fff', borderRadius:'8px', padding:'4px 8px', cursor:'pointer'}); closeBtn.addEventListener('click', closeWardDrawer);
    drawerTitle = document.createElement('div'); Object.assign(drawerTitle.style,{fontWeight:'700', fontSize:'14px', flex:'1 1 auto'});
    header.appendChild(closeBtn); header.appendChild(drawerTitle); drawer.appendChild(header);

    // controls
    const controls = document.createElement('div'); Object.assign(controls.style,{padding:'8px 12px', borderBottom:'1px solid #eee', display:'grid', gridTemplateColumns:'1fr 1fr', gap:'8px'});
    ...
    drawerAnsSel = document.createElement('select'); for(const v of ['(Any answer)','Yes','No','Undecided']){ const o=document.createElement('option'); o.value=(v==='(Any answer)')?'':v; o.textContent=v; drawerAnsSel.appendChild(o); }
    ...
    controls.appendChild(drawerIssueSel); controls.appendChild(drawerAnsSel); controls.appendChild(drawerSearch); drawer.appendChild(controls);

    // table
    const tableWrap = document.createElement('div'); Object.assign(tableWrap.style,{flex:'1 1 auto', overflow:'auto'});
    ...
    table.innerHTML = `
      <thead style="position:sticky;top:0;background:#fafafa"><tr>
        <th style="text-align:left;border-bottom:1px solid #eee;padding:6px 8px;white-space:nowrap">Candidate</th>
        <th style="text-align:left;border-bottom:1px solid #eee;padding:6px 8px">Issue</th>
        <th style="text-align:left;border-bottom:1px solid #eee;padding:6px 8px;white-space:nowrap">Answer</th>
        <th style="text-align:left;border-bottom:1px solid #eee;padding:6px 8px">Comment</th>
      </tr></thead><tbody></tbody>`;
    drawerTableBody = table.querySelector('tbody'); tableWrap.appendChild(table); drawer.appendChild(tableWrap);

    // listeners
    const rerender = ()=> renderWardDrawer(currentWardKey);
    drawerIssueSel.addEventListener('change', rerender); drawerAnsSel.addEventListener('change', rerender); drawerSearch.addEventListener('input', rerender);

    // expose API for popup buttons
  ...
  function openWardDrawer(wardKey){
    ...
    drawerTitle.textContent = wc ? `All responses — ${wc.name}` : `All responses — Ward ${currentWardKey}`;
    drawerIssueSel.value=''; drawerAnsSel.value=''; drawerSearch.value='';
  ...
  function renderWardDrawer(wardKey){
    if(!wardKey){ drawerTableBody.innerHTML=''; return; }
    const rows = (currentFilteredByWard.get(wardKey) || []);
    const issueFilter = drawerIssueSel.value || '';
    const ansFilter = (drawerAnsSel.value || '').toLowerCase();
    const q = lc(drawerSearch.value||'').trim();
    ...
    for(const r of rows){
      ...
      for(const issue of survey.issueColumns){
        ...
        fragments.push(`
          <tr>
            <td style="border-bottom:1px solid #f0f0f0;padding:6px 8px;white-space:nowrap">${name}</td>
            <td style="border-bottom:1px solid #f0f0f0;padding:6px 8px">${escapeHtml(issue)}</td>
            <td style="border-bottom:1px solid #f0f0f0;padding:6px 8px;white-space:nowrap">
            ...
            <td style="border-bottom:1px solid #f0f0f0;padding:6px 8px">${escapeHtml(comment||'')}</td>
          </tr>`);
      }
    }
    drawerTableBody.innerHTML = fragments.length ? fragments.join('') : `<tr><td colspan="4" style="padding:10px 8px;color:#666"><i>No rows match the drawer filters.</i></td></tr>`;
  }
